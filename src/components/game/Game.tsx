...
export default function Game() {
  ...
  useEffect(() => {
    ...
    const AUTO_SAVE_MS = 5 * 60 * 1000 // 5 minutos

    const interval = setInterval(() => {
      const state = stateRef.current
      if (!state.player || state.screen !== 'playing') return

      // Salva localmente (funciona offline e sem conta)
      saveGame(state)

      // Se logado, salva também no sistema de contas
      if (account) {
        startAutoSave(
          () => ({
            ...
            playtime: Math.floor(state.tick / 60),
      ...
      // Notificação visual de auto-save (remove duplicatas antes de adicionar)
      setGameState((prev) => {
        const filtered = prev.notifications.filter(n => !n.id.startsWith('autosave_'))
        return {
          ...prev,
          notifications: [
            ...filtered,
            {
              ...
              text: '💾 Jogo salvo automaticamente',
              type: 'achievement' as const,
              timer: 180,
            },
          ].slice(-20), // cap at 20 notifications to avoid memory leak
        }
      })
    }, AUTO_SAVE_MS)
  ...
  }, [account, gameState.screen])

  useEffect(() => {
    if (gameState.screen !== 'playing') return

    let lastTime = performance.now()
    let hidden = false // Android WebView may report document.hidden=true on startup
    const onVis = () => {
      hidden = document.hidden
      lastTime = performance.now() // reset to avoid huge dt after background
      if (!document.hidden) resumeAudio() // re-resume audio context on Android
    }
    document.addEventListener('visibilitychange', onVis)
    // Capacitor / Android: 'resume' fires when app comes back from background
    const onResume = () => { hidden = false; lastTime = performance.now(); resumeAudio() }
    ...
    const loop = (now: number) => {
      ...
      // CORRIGIDO: dt < 200 (era 100) para não pular frames em dispositivos lentos;
      // também processa quando dt >= 100 para evitar tela preta (nunca skippa sempre).
      if (!hidden && (dt < 200 || dt >= 100)) {
        setGameState((prev) => {
  ...
  useEffect(() => {
    const onDown = (e: KeyboardEvent) => {
      ...
      if (!stateRef.current.editorOpen && !stateRef.current.isPaused) {
        const slotMatch = /^Digit([1-5])$/.exec(e.code)
        if (slotMatch) {
    ...
    <div className="w-full h-full relative overflow-hidden" style={{ background: '#080a0e' }}>
      ...
      {!gameState.editorOpen && gameState.player && gameState.currentMap && !isDead && (
        <RucoyModalBar
          ...
          onToggleSkinShop={() => setShowSkinShop(v => !v)}
          onToggleEnemyCatalog={() => setShowEnemyCatalog(v => !v)}
          ...
          onLoadSave={(slotId) => {
            ...
            if (slot) {
              try {
                ...
                // Ensure the loaded state has all required fields to avoid crashes
                setGameState({
                  ...loadedState,
                  damageNumbers: loadedState.damageNumbers ?? [],
                  particles: loadedState.particles ?? [],
                  minions: loadedState.minions ?? [],
                  projectiles: loadedState.projectiles ?? [],
                  areaEffects: loadedState.areaEffects ?? [],
                  notifications: loadedState.notifications ?? [],
                  chatMessages: loadedState.chatMessages ?? [],
                })
              } catch (e) {
      ...
      {/* Skin Shop */}
      {showSkinShop && !gameState.editorOpen && gameState.player && (
        <SkinShop
          player={gameState.player}
          onClose={() => setShowSkinShop(false)}
          onBuySkin={(skin) => setGameState((prev) => {
            if (!prev.player) return prev
            const cls = prev.player.class
            const owned = (prev.player as any)._ownedLegendarySkins as string[] | undefined
            const newOwned = owned ? [...owned] : []
            if (!newOwned.includes(cls)) newOwned.push(cls)
            const isFree = cls === 'necromancer'
            const cost = isFree ? 0 : 100
            const gems = (prev.player.gems ?? 0) - cost
            return { ...prev, player: { ...prev.player, gems: Math.max(0, gems), skin, ...( { _ownedLegendarySkins: newOwned } as any) } }
          })}
          onEquipSkin={(skin) => setGameState((prev) => prev.player ? { ...prev, player: { ...prev.player, skin } } : prev)}
        />
      )}
      {/* Enemy Catalog */}
      {showEnemyCatalog && !gameState.editorOpen && (
        <EnemyCatalog onClose={() => setShowEnemyCatalog(false)} />
      )}

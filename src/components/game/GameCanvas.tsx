...
export default function GameCanvas({ gameState, onCanvasClick }: Props) {
  ...
  const render = useCallback(() => {
    ...
    if (chunksBuiltThisFrame < CHUNK_BUDGET) {
      ...
      if (pCX < Math.ceil(currentMap.width / CHUNK_TILES) && !cache.chunks.has(pkey)) {
        // Cap chunk cache to avoid GPU memory exhaustion on mobile
        if (cache.chunks.size < 200) {
    ...
    // ── Biome Transition Overlay (west biomes) ──
    // When player is on the 'city' map and in the western biome zones, show a
    // smooth colour-tinted gradient at each biome boundary so transitions feel
    // seamless instead of abrupt.
    if (currentMap.id === 'city') {
      const WEST_BIOME_W = 60   // tiles per biome (same as data.ts)
      const WEST_BIOME_COUNT = 10
      const BIOME_COLORS = [
        [80, 200, 80],   // Pradaria Esmeralda — green
        [210, 160, 60],  // Deserto Ardente — sand
        [70, 110, 60],   // Pântano Lúgubre — dark green
        [160, 200, 230], // Tundra Glacial — ice blue
        [200, 80, 40],   // Vulcão Caótico — red-orange
        [30, 0, 60],     // Abismo Sombrio — purple-black
        [140, 80, 220],  // Cavernas Cristalinas — purple
        [100, 140, 120], // Ruínas Etéreas — teal gray
        [200, 220, 255], // Reino Celeste — pale blue
        [40, 100, 30],   // Floresta Eterna — deep green
      ]
      // Player tile position
      const ptx = Math.floor(player.position.x / 32)
      const pty = Math.floor(player.position.y / 32)
      // Which biome are we in? (biome 0 is westernmost)
      const biomeIdx = Math.floor(ptx / WEST_BIOME_W)
      if (biomeIdx >= 0 && biomeIdx < WEST_BIOME_COUNT) {
        const col = BIOME_COLORS[biomeIdx]
        // Distance into this biome (0..1)
        const localX = (ptx % WEST_BIOME_W) / WEST_BIOME_W
        // Fade in near left edge, fade out near right edge
        const FADE_ZONE = 0.18
        let alpha = 0
        if (localX < FADE_ZONE) {
          // Entering from the left: cross-fade from previous biome
          alpha = (localX / FADE_ZONE) * 0.22
          if (biomeIdx > 0) {
            const prevCol = BIOME_COLORS[biomeIdx - 1]
            const prevA = (1 - localX / FADE_ZONE) * 0.22
            ctx.fillStyle = `rgba(${prevCol[0]},${prevCol[1]},${prevCol[2]},${prevA})`
            ctx.fillRect(0, 0, canvas.width, canvas.height)
          }
        } else if (localX > 1 - FADE_ZONE) {
          // Approaching right edge: cross-fade to next biome
          alpha = ((1 - localX) / FADE_ZONE) * 0.22
          if (biomeIdx < WEST_BIOME_COUNT - 1) {
            const nextCol = BIOME_COLORS[biomeIdx + 1]
            const nextA = (1 - (1 - localX) / FADE_ZONE) * 0.22
            ctx.fillStyle = `rgba(${nextCol[0]},${nextCol[1]},${nextCol[2]},${nextA})`
            ctx.fillRect(0, 0, canvas.width, canvas.height)
          }
        } else {
          alpha = 0.12
  ...
  // ── Context loss recovery (GPU memory exhaustion on mobile) ──────────────
  useEffect(() => {
    const canvas = canvasRef.current
    if (!canvas) return
    const onLost = (e: Event) => {
      e.preventDefault()
      ctxRef.current = null // force re-acquisition on next render
    }
    const onRestored = () => {
      ctxRef.current = null // reset so getContext() is called fresh
      render()
    }
  ...
  // ── Force re-render when app returns to foreground (Android WebView) ──────
  useEffect(() => {
    const onVis = () => {
      if (!document.hidden) {
        // Small delay to let WebView fully restore GPU context

...
function MiniMapImpl({ gameState, onToggleWorldMap }: Props) {
  ...
  useEffect(() => {
    ...
    // Portal overlay (blue, with glow) — drawn on top so portals are always visible
    const PORTAL_TILES = new Set([
      'portal', 'crystal_portal', 'haunted_portal', 'sky_portal',
      'mountain_portal', 'ruins_portal',
    ])
    ...
    for (let y = 0; y < mapH; y++) {
      for (let x = 0; x < mapW; x++) {
        const tile = currentMap.tiles[y]?.[x]
        if (tile && PORTAL_TILES.has(tile.type)) {
          ctx.fillRect(x * scaleX - 0.5, y * scaleY - 0.5, scaleX + 1.5, scaleY + 1.5)
        }
      }
    }
    ctx.shadowBlur = 0
...
const TILE_COLORS: Record<string, string> = {
  ...
  chest: '#4a3a0a', portal: '#40a8ff',
  crystal_portal: '#40a8ff', haunted_portal: '#40a8ff', sky_portal: '#40a8ff',
  mountain_portal: '#40a8ff', ruins_portal: '#40a8ff',

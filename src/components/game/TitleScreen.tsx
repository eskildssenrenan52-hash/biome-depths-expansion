...
export default function TitleScreen({ onStart, onLoad }: Props) {
  ...
  const [screen, setScreen] = useState<'title' | 'select' | 'create' | 'auth' | 'register'>('title')
  ...
  useEffect(() => {
    supabase.auth.getSession().then(({ data, error }) => {
      if (error) {
        console.warn('[TitleScreen] getSession error:', error.message)
        // Stay on title screen in offline mode
        return
      }
      setSession(data.session)
      if (data.session) setScreen('title')
      // No session = stay on title (guest mode)
    }).catch((e) => {
      console.warn('[TitleScreen] getSession failed:', e)
      // Stay on title screen in offline mode
    })
    const { data: sub } = supabase.auth.onAuthStateChange((_evt, s) => {
      ...
      if (s) {
        ...
        setUsername('')
        setSlots([])
        // Don't redirect to auth — keep user on title in guest/offline mode
      }
    })
  ...
  useEffect(() => {
    ...
    fetchCloudSlots().then((s) => {
      ...
      console.warn('[TitleScreen] fetchCloudSlots failed:', e)
      setSlots([])
      setSlotsLoading(false)
    })
  }, [session, screen])

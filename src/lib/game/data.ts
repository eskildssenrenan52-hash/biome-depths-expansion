...
function generateCityMap(): GameMap {
  ...
  // ─── 2 NOVOS BIOMAS (Lv50+) à direita da floresta — portais visíveis ────
  // Refúgio Solar e Santuário das Brumas (2 andares cada, generated under map IDs solhaven1/2, mistshrine1/2)
  const eastBiomePortals = [
    { px: eastStart + 80,  py: NORTH + 150 },  // tx=580, ty=150 → solhaven1
    { px: eastStart + 180, py: NORTH + 350 },  // tx=680, ty=350 → mistshrine1
  ]
  for (const p of eastBiomePortals) {
    for (let dy = -2; dy <= 2; dy++) {
      for (let dx = -2; dx <= 2; dx++) {
        const x = p.px + dx, y = p.py + dy
        if (x >= 0 && x < expW && y >= 0 && y < newH) expTiles[y][x] = makeTile('cobblestone')
      }
    }
    expTiles[p.py][p.px] = makeTile('portal')
  }


  // ─── 10 BIOMAS COM 3 ANDARES AO OESTE — DESIGN RICO E ÚNICO ──────────────
  ...
  // Cada bioma tem um gerador próprio com padrões visuais únicos.
  const BIOMES: {
    name: string
    base: TileType
    ...
    { name: 'Pradaria Esmeralda',  base: 'grass',      accents: [{ tile: 'flower', chance: 0.12 }, { tile: 'tall_grass', chance: 0.08 }], mobs: ['slime', 'wolf', 'goblin', 'prism_wolf'],                  levels: [3, 8, 15] },
    { name: 'Deserto Ardente',     base: 'sand',       accents: [{ tile: 'rock', chance: 0.10 }, { tile: 'dirt', chance: 0.08 }],         mobs: ['skeleton', 'spider', 'goblin', 'amber_warden'],            levels: [6, 14, 22] },
    { name: 'Pantano Lugubre',     base: 'dirt',       accents: [{ tile: 'water', chance: 0.18 }, { tile: 'tall_grass', chance: 0.10 }],  mobs: ['zombie', 'witch', 'spider', 'coral_ghoul', 'rot_sage'],                levels: [8, 18, 28] },
    { name: 'Tundra Glacial',      base: 'snow',       accents: [{ tile: 'ice', chance: 0.10 }, { tile: 'frozen_tree', chance: 0.10 }],   mobs: ['ghost', 'troll', 'wolf', 'frost_sentinel', 'living_tempest'],                   levels: [10, 22, 34] },
    { name: 'Vulcao Caotico',      base: 'volcanic_rock', accents: [{ tile: 'lava', chance: 0.12 }, { tile: 'ash', chance: 0.10 }],       mobs: ['demon', 'witch', 'troll', 'ember_colossus'],                  levels: [14, 28, 42] },
    { name: 'Abismo Sombrio',      base: 'abyss_floor', accents: [{ tile: 'void', chance: 0.18 }, { tile: 'soul_fire', chance: 0.04 }],   mobs: ['ghost', 'demon', 'vampire', 'echo_specter'],                levels: [18, 34, 50] },
    { name: 'Cavernas Cristalinas',base: 'crystal_floor', accents: [{ tile: 'crystal', chance: 0.10 }, { tile: 'gem_node', chance: 0.04 }], mobs: ['skeleton', 'orc', 'troll', 'prism_wolf', 'rune_devourer'],              levels: [12, 24, 38] },
    { name: 'Ruinas Etereas',      base: 'ruin_floor', accents: [{ tile: 'cobweb', chance: 0.06 }, { tile: 'ruin_wall', chance: 0.06 }],  mobs: ['ghost', 'vampire', 'witch', 'echo_specter', 'rune_devourer'],                levels: [16, 30, 46] },
    { name: 'Reino Celeste',       base: 'cloud_floor', accents: [{ tile: 'sky_platform', chance: 0.18 }, { tile: 'sky_void', chance: 0.12 }], mobs: ['demon', 'dragon', 'witch', 'living_tempest'],            levels: [22, 38, 56] },
    { name: 'Floresta Eterna',     base: 'ancient_bark', accents: [{ tile: 'mossy_stone', chance: 0.10 }, { tile: 'mushroom', chance: 0.06 }, { tile: 'tree', chance: 0.10 }], mobs: ['treant', 'witch', 'spider', 'broodmother', 'rot_sage'], levels: [20, 36, 52] },
  ]
  ...
  for (let b = 0; b < WEST_BIOME_COUNT; b++) {
    ...
    // preenche bioma com gerador único por bioma
    for (let y = 0; y < newH; y++) {
      for (let x = x0; x < x1; x++) {
        const lx = x - x0
        let t: TileType = def.base
        // Cada bioma tem um padrão de geração único
        switch (b) {
          case 0: { // Pradaria Esmeralda — ondas de grama com clareiras
            const wave = Math.sin(lx * 0.3 + y * 0.15) * 0.5 + 0.5
            if (wave > 0.85) t = 'flower'
            else if (wave > 0.7) t = 'tall_grass'
            else if ((lx * 7 + y * 13) % 47 === 0) t = 'tree'
            else if ((lx * 3 + y * 5) % 31 === 0) t = 'flower'
            else t = 'grass'
            break
          }
          case 1: { // Deserto Ardente — dunas com rochas e ossadas
            const dune = Math.sin(lx * 0.2) * Math.cos(y * 0.1) * 0.5 + 0.5
            if (dune > 0.8) t = 'rock'
            else if (dune < 0.2) t = 'dirt'
            else if ((lx * 11 + y * 7) % 53 === 0) t = 'rock'
            else if ((lx * 5 + y * 3) % 37 === 0) t = 'dirt'
            else t = 'sand'
            break
          }
          case 2: { // Pântano Lúgubre — poças e vegetação rasteira
            const bog = Math.sin(lx * 0.25 + y * 0.2) * 0.5 + 0.5
            if (bog > 0.75) t = 'water'
            else if (bog > 0.6) t = 'tall_grass'
            else if ((lx * 9 + y * 5) % 41 === 0) t = 'water'
            else if ((lx * 4 + y * 7) % 29 === 0) t = 'tall_grass'
            else t = 'dirt'
            break
          }
          case 3: { // Tundra Glacial — campos de neve com gelo e árvores congeladas
            const frost = Math.sin(lx * 0.15 + y * 0.25) * 0.5 + 0.5
            if (frost > 0.82) t = 'ice'
            else if (frost > 0.7) t = 'frozen_tree'
            else if ((lx * 13 + y * 3) % 59 === 0) t = 'ice_rock'
            else if ((lx * 5 + y * 11) % 43 === 0) t = 'frozen_tree'
            else t = 'snow'
            break
          }
          case 4: { // Vulcão Caótico — rachaduras de lava e cinzas
            const heat = Math.sin(lx * 0.3 + y * 0.2) * 0.5 + 0.5
            if (heat > 0.88) t = 'lava'
            else if (heat > 0.75) t = 'ash'
            else if (heat < 0.15) t = 'ash'
            else if ((lx * 7 + y * 11) % 47 === 0) t = 'lava'
            else if ((lx * 3 + y * 5) % 23 === 0) t = 'obsidian'
            else t = 'volcanic_rock'
            break
          }
          case 5: { // Abismo Sombrio — vazio e fogo alma
            const voidVal = Math.sin(lx * 0.2 + y * 0.3) * 0.5 + 0.5
            if (voidVal > 0.85) t = 'void'
            else if (voidVal > 0.78) t = 'soul_fire'
            else if ((lx * 11 + y * 7) % 61 === 0) t = 'soul_fire'
            else if ((lx * 5 + y * 13) % 37 === 0) t = 'dark_crystal'
            else t = 'abyss_floor'
            break
          }
          case 6: { // Cavernas Cristalinas — formações de cristal
            const cryst = Math.sin(lx * 0.25 + y * 0.15) * 0.5 + 0.5
            if (cryst > 0.85) t = 'crystal'
            else if (cryst > 0.75) t = 'gem_node'
            else if ((lx * 9 + y * 5) % 43 === 0) t = 'crystal'
            else if ((lx * 4 + y * 9) % 31 === 0) t = 'gem_node'
            else t = 'crystal_floor'
            break
          }
          case 7: { // Ruínas Etéas — pilares e teias
            const ruin = Math.sin(lx * 0.2 + y * 0.2) * 0.5 + 0.5
            if (ruin > 0.82) t = 'ruin_wall'
            else if (ruin > 0.7) t = 'cobweb'
            else if ((lx * 7 + y * 13) % 41 === 0) t = 'ruin_pillar'
            else if ((lx * 5 + y * 3) % 29 === 0) t = 'cobweb'
            else t = 'ruin_floor'
            break
          }
          case 8: { // Reino Celeste — plataformas flutuantes e vazio celestial
            const sky = Math.sin(lx * 0.15 + y * 0.2) * 0.5 + 0.5
            if (sky > 0.8) t = 'sky_platform'
            else if (sky < 0.2) t = 'sky_void'
            else if ((lx * 11 + y * 5) % 47 === 0) t = 'sky_platform'
            else if ((lx * 3 + y * 7) % 37 === 0) t = 'sky_void'
            else t = 'cloud_floor'
            break
          }
          case 9: { // Floresta Eterna — árvores antigas e musgo
            const forest = Math.sin(lx * 0.2 + y * 0.25) * 0.5 + 0.5
            if (forest > 0.82) t = 'tree'
            else if (forest > 0.7) t = 'mossy_stone'
            else if ((lx * 9 + y * 7) % 43 === 0) t = 'mushroom'
            else if ((lx * 5 + y * 11) % 31 === 0) t = 'mossy_stone'
            else if ((lx * 3 + y * 4) % 19 === 0) t = 'tree'
            else t = 'ancient_bark'
            break
          }
        }
    ...
    // Features decorativas especiais por bioma
    const cx0 = x0 + Math.floor(WEST_BIOME_W / 2)
    switch (b) {
      case 0: { // Pradaria — lago central + clareira de flores
        const lakeR = 8
        for (let dy = -lakeR; dy <= lakeR; dy++) {
          for (let dx = -lakeR; dx <= lakeR; dx++) {
            if (dx*dx + dy*dy <= lakeR*lakeR) {
              const ly = floorMidY + dy, lx = cx0 + dx
              if (ly > 0 && ly < newH && lx > x0 && lx < x1 - 1)
                expTiles[ly][lx] = makeTile('water')
            }
          }
        }
        // Anel de flores ao redor do lago
        for (let a = 0; a < 360; a += 15) {
          const fx = Math.floor(cx0 + Math.cos(a * Math.PI/180) * (lakeR + 2))
          const fy = Math.floor(floorMidY + Math.sin(a * Math.PI/180) * (lakeR + 2))
          if (fy > 0 && fy < newH && fx > x0 && fx < x1 - 1)
            expTiles[fy][fx] = makeTile('flower')
        }
        break
      }
      case 1: { // Deserto — oasis no andar do meio
        const oasisR = 6
        for (let dy = -oasisR; dy <= oasisR; dy++) {
          for (let dx = -oasisR; dx <= oasisR; dx++) {
            if (dx*dx + dy*dy <= oasisR*oasisR) {
              const oy = floorMidY + dy, ox = cx0 + dx
              if (oy > 0 && oy < newH && ox > x0 && ox < x1 - 1)
                expTiles[oy][ox] = makeTile(dx*dx + dy*dy <= 4 ? 'water' : 'grass')
            }
          }
        }
        // Palmeiras ao redor
        for (let a = 0; a < 360; a += 30) {
          const px = Math.floor(cx0 + Math.cos(a * Math.PI/180) * (oasisR + 1))
          const py = Math.floor(floorMidY + Math.sin(a * Math.PI/180) * (oasisR + 1))
          if (py > 0 && py < newH && px > x0 && px < x1 - 1)
            expTiles[py][px] = makeTile('tree')
        }
        break
      }
      case 2: { // Pântano — árvores mortas e poças grandes
        for (let i = 0; i < 8; i++) {
          const px = x0 + 5 + Math.floor(Math.random() * (WEST_BIOME_W - 10))
          const py = floorMidY + Math.floor(Math.random() * floorH) - Math.floor(floorH/2)
          if (py > 0 && py < newH) expTiles[py][px] = makeTile('tree')
        }
        // Poça grande no andar do topo
        const pondR = 7
        for (let dy = -pondR; dy <= pondR; dy++) {
          for (let dx = -pondR; dx <= pondR; dx++) {
            if (dx*dx + dy*dy <= pondR*pondR) {
              const py = floorTopY + Math.floor(floorH/2) + dy, px = cx0 + dx
              if (py > 0 && py < newH && px > x0 && px < x1 - 1)
                expTiles[py][px] = makeTile('water')
            }
          }
        }
        break
      }
      case 3: { // Tundra — igre de gelo central
        const iglooR = 5
        for (let dy = -iglooR; dy <= iglooR; dy++) {
          for (let dx = -iglooR; dx <= iglooR; dx++) {
            if (dx*dx + dy*dy <= iglooR*iglooR) {
              const iy = floorMidY + dy, ix = cx0 + dx
              if (iy > 0 && iy < newH && ix > x0 && ix < x1 - 1)
                expTiles[iy][ix] = makeTile('ice')
            }
          }
        }
        // Árvores congeladas em padrão radial
        for (let a = 0; a < 360; a += 20) {
          const fx = Math.floor(cx0 + Math.cos(a * Math.PI/180) * (iglooR + 3))
          const fy = Math.floor(floorMidY + Math.sin(a * Math.PI/180) * (iglooR + 3))
          if (fy > 0 && fy < newH && fx > x0 && fx < x1 - 1)
            expTiles[fy][fx] = makeTile('frozen_tree')
        }
        break
      }
      case 4: { // Vulcão — cratera de lava central
        const craterR = 7
        for (let dy = -craterR; dy <= craterR; dy++) {
          for (let dx = -craterR; dx <= craterR; dx++) {
            const dist = Math.sqrt(dx*dx + dy*dy)
            if (dist <= craterR) {
              const vy = floorMidY + dy, vx = cx0 + dx
              if (vy > 0 && vy < newH && vx > x0 && vx < x1 - 1)
                expTiles[vy][vx] = makeTile(dist <= 3 ? 'lava' : 'magma_crust')
            }
          }
        }
        // Ventos vulcânicos
        for (let i = 0; i < 4; i++) {
          const vx = x0 + 5 + Math.floor(Math.random() * (WEST_BIOME_W - 10))
          const vy = floorTopY + 5 + Math.floor(Math.random() * (newH - 10))
          if (vy > 0 && vy < newH) expTiles[vy][vx] = makeTile('volcanic_vent')
        }
        break
      }
      case 5: { // Abismo — pilar de alma central
        const soulR = 5
        for (let dy = -soulR; dy <= soulR; dy++) {
          for (let dx = -soulR; dx <= soulR; dx++) {
            if (dx*dx + dy*dy <= soulR*soulR) {
              const sy = floorMidY + dy, sx = cx0 + dx
              if (sy > 0 && sy < newH && sx > x0 && sx < x1 - 1)
                expTiles[sy][sx] = makeTile(dx*dx + dy*dy <= 2 ? 'soul_fire' : 'void')
            }
          }
        }
        // Cristais escuros espalhados
        for (let i = 0; i < 6; i++) {
          const dx = x0 + 3 + Math.floor(Math.random() * (WEST_BIOME_W - 6))
          const dy = floorTopY + 3 + Math.floor(Math.random() * (newH - 6))
          if (dy > 0 && dy < newH) expTiles[dy][dx] = makeTile('dark_crystal')
        }
        break
      }
      case 6: { // Cavernas Cristalinas — cluster de cristais gigantes
        for (let i = 0; i < 5; i++) {
          const cx = x0 + 5 + Math.floor(Math.random() * (WEST_BIOME_W - 10))
          const cy = floorMidY + Math.floor(Math.random() * floorH) - Math.floor(floorH/2)
          if (cy > 0 && cy < newH) {
            // Cluster de cristais
            for (let dy = -2; dy <= 2; dy++) {
              for (let dx = -2; dx <= 2; dx++) {
                if (Math.abs(dx) + Math.abs(dy) <= 2) {
                  ...
                  if (py > 0 && py < newH && px > x0 && px < x1 - 1)
                    expTiles[py][px] = makeTile(Math.random() < 0.4 ? 'gem_node' : 'crystal')
                }
              }
            }
          }
      ...
      case 7: { // Ruínas Etéas — círculo de pilares
        const ringR = 8
        for (let a = 0; a < 360; a += 45) {
          const px = Math.floor(cx0 + Math.cos(a * Math.PI/180) * ringR)
          const py = Math.floor(floorMidY + Math.sin(a * Math.PI/180) * ringR)
          if (py > 0 && py < newH && px > x0 && px < x1 - 1)
            expTiles[py][px] = makeTile('ruin_pillar')
        }
        // Centro: sarcófago
        expTiles[floorMidY][cx0] = makeTile('sarcophagus')
        // Teias nos cantos
        for (let i = 0; i < 6; i++) {
          const wx = x0 + 2 + Math.floor(Math.random() * (WEST_BIOME_W - 4))
          const wy = floorTopY + 2 + Math.floor(Math.random() * (newH - 4))
          if (wy > 0 && wy < newH) expTiles[wy][wx] = makeTile('cobweb')
        }
        break
      }
      case 8: { // Reino Celeste — ilhas flutuantes
        for (let i = 0; i < 4; i++) {
          const ix = x0 + 8 + Math.floor(Math.random() * (WEST_BIOME_W - 16))
          const iy = floorTopY + 10 + Math.floor(Math.random() * (newH - 20))
          if (iy > 0 && iy < newH) {
            const islandR = 4 + Math.floor(Math.random() * 3)
            for (let dy = -islandR; dy <= islandR; dy++) {
              for (let dx = -islandR; dx <= islandR; dx++) {
                if (dx*dx + dy*dy <= islandR*islandR) {
                  const py = iy + dy, px = ix + dx
                  if (py > 0 && py < newH && px > x0 && px < x1 - 1)
                    expTiles[py][px] = makeTile('sky_platform')
                }
              }
            }
          }
      ...
      case 9: { // Floresta Eterna — clareira com cogumelos gigantes
        const clearingR = 6
        for (let dy = -clearingR; dy <= clearingR; dy++) {
          for (let dx = -clearingR; dx <= clearingR; dx++) {
            if (dx*dx + dy*dy <= clearingR*clearingR) {
              const fy = floorMidY + dy, fx = cx0 + dx
              if (fy > 0 && fy < newH && fx > x0 && fx < x1 - 1)
                expTiles[fy][fx] = makeTile('mossy_stone')
            }
          }
        }
        // Cogumelos ao redor
        for (let a = 0; a < 360; a += 30) {
          const mx = Math.floor(cx0 + Math.cos(a * Math.PI/180) * (clearingR + 2))
          const my = Math.floor(floorMidY + Math.sin(a * Math.PI/180) * (clearingR + 2))
          if (my > 0 && my < newH && mx > x0 && mx < x1 - 1)
            expTiles[my][mx] = makeTile('mushroom')
        }
        // Árvores densas nas bordas
        for (let i = 0; i < 10; i++) {
          const tx = x0 + 2 + Math.floor(Math.random() * (WEST_BIOME_W - 4))
          const ty = floorTopY + 2 + Math.floor(Math.random() * (newH - 4))
          if (ty > 0 && ty < newH && expTiles[ty][tx].type !== 'mossy_stone')
            expTiles[ty][tx] = makeTile('tree')
        }
        break
      }
    }
    // SEM muros verticais entre biomas — transição contínua e suave
    // A fronteira visual é tratada pelo overlay de transição no GameCanvas
    ...
    // ─── Promenade central organizada: caminho de pedra vertical conectando
    //     todas as 3 entradas (top, mid, bot) por dentro do bioma ──────────
    const promX = x0 + Math.floor(WEST_BIOME_W / 2)
    for (let py = 4; py < newH - 4; py++) {
      // não sobrescreve as muralhas dos andares
      if (py === floorMidY || py === floorBotY) continue
      // caminho largura 2 (cobblestone)
      for (let dx = -1; dx <= 1; dx++) {
        const cx = promX + dx
        if (cx > x0 && cx < x1 - 1) expTiles[py][cx] = makeTile('cobblestone')
      }
    }
    // pequena praça circular no centro de cada andar
    for (const fy of [floorTopY + Math.floor(floorH / 2), floorMidY + Math.floor(floorH / 2), floorBotY + Math.floor(floorH / 2)]) {
      for (let dy = -2; dy <= 2; dy++) for (let dx = -2; dx <= 2; dx++) {
        if (dx * dx + dy * dy <= 4) {
          const sx = promX + dx, sy = fy + dy
          if (sx > x0 && sx < x1 - 1 && sy > 0 && sy < newH - 1) expTiles[sy][sx] = makeTile('cobblestone')
        }
      }
      // marco central distinto por bioma (lampião)
      if (fy > 0 && fy < newH) expTiles[fy][promX] = makeTile('lamp_post')
    }

    // spawn mobs por andar (não pisa em cobblestone, só em terreno walkable original)
...
export function makeClassProgress(cls: CharacterClass): import('./types').ClassProgress {
  return {
    level: 1,
    xp: 0,
    xpToNext: 100,
...
export function recalcStats(player: Player): CharacterStats {
  ...
  // Defense skill bonus (skill index 1 = "Defesa")
  const defSkill = player.skills?.[1]
  if (defSkill && defSkill.level > 1) {
    base.defense = base.defense + (defSkill.level - 1) * 3
  }
  // Vitality skill bonus (skill index 2 = "Vitalidade")
  const vitSkill = player.skills?.[2]
  if (vitSkill && vitSkill.level > 1) {
    base.maxHp = base.maxHp + (vitSkill.level - 1) * 12
  }

...
// ═══════════════════════════════════════════════════════════════════════════════
// REFÚGIO SOLAR — oásis dourado banhado pelo sol (Lv50+)
// ═══════════════════════════════════════════════════════════════════════════════
function genSunHaven(f: 1 | 2): GameMap {
  const W=300, H=300, rand=rngOf(8800+f*53)
  const base: TileType = f===1 ? 'sand' : 'magma_crust'
  const t = blank(W, H, base)
  // Caminhos solares irradiando do centro
  const cx = Math.floor(W/2), cy = Math.floor(H/2)
  for (let a=0; a<360; a+=30) {
    const rd=a*Math.PI/180
    for (let r=0; r<Math.floor(W/2)-6; r++) {
      const x=Math.round(cx+Math.cos(rd)*r), y=Math.round(cy+Math.sin(rd)*r)
      if (x>1&&y>1&&x<W-1&&y<H-1) t[y][x] = T(f===1?'cobblestone':'obsidian')
    }
  }
  // Lago central com flores douradas
  disc(t, cx, cy, 10, f===1?'water':'lava')
  ring(t, cx, cy, 12, f===1?'flower':'soul_fire')
  // Oásis menores em padrão hexagonal
  for (let i=0; i<6; i++) {
    const ang=i*60*Math.PI/180, rr=70
    const ox=Math.round(cx+Math.cos(ang)*rr), oy=Math.round(cy+Math.sin(ang)*rr)
    disc(t, ox, oy, 6, f===1?'grass':'crystal_floor')
    ring(t, ox, oy, 7, f===1?'tree':'gem_node')
    if (f===2) t[oy][ox] = T('soul_fire')
  }
  // Pilares dourados (organizados em grid 4x4 nos quadrantes)
  for (let i=0; i<4; i++) for (let j=0; j<4; j++) {
    const px = 30 + i*60, py = 30 + j*60
    if (Math.hypot(px-cx, py-cy) > 25 && px<W-2 && py<H-2) {
      t[py][px] = T(f===1?'ruin_pillar':'dark_crystal')
    }
  }
  border(t, W, H, f===1?'rock':'volcanic_rock')
  portalAt(t, 8, 8, 'portal', f===1?'sand':'magma_crust')
  if (f===1) portalAt(t, W-9, H-9, 'crystal_portal', 'cobblestone')
  const pool: MonsterType[] = f===1
    ? ['demon','dragon','witch','vampire']
    : ['dragon','demon','vampire','ember_colossus' as MonsterType]
  const monsters = spawnPack(t, rand, 90, f===1?50:62, pool, 0.18)
  if (f===2) monsters.push(createMonster('dragon', 75, cx*32, cy*32, 'boss'))
  return { id: `solhaven${f}`, name: `Refúgio Solar — Andar ${f}`, width: W, height: H, tiles: t, monsters,
    spawnPoints: [{x:10*32, y:10*32}], ambience: f===1?'desert':'volcano', musicTheme: f===1?'desert':'volcano',
    minLevel: f===1?50:62 }
}

// ═══════════════════════════════════════════════════════════════════════════════
// SANTUÁRIO DAS BRUMAS — templo etéreo envolto em névoa cristalina (Lv55+)
// ═══════════════════════════════════════════════════════════════════════════════
function genMistShrine(f: 1 | 2): GameMap {
  const W=300, H=300, rand=rngOf(8900+f*59)
  const base: TileType = f===1 ? 'cloud_floor' : 'abyss_floor'
  const t = blank(W, H, base)
  // Plataformas concêntricas
  const cx = Math.floor(W/2), cy = Math.floor(H/2)
  for (let r=20; r<=120; r+=20) {
    ring(t, cx, cy, r,   f===1?'sky_platform':'crystal_wall')
    ring(t, cx, cy, r+1, f===1?'sky_platform':'crystal_wall')
  }
  // Aberturas radiais nas plataformas (4 portões cardeais por anel)
  for (let r=20; r<=120; r+=20) {
    for (const ang of [0, 90, 180, 270]) {
      const rd=ang*Math.PI/180
      for (let dr=-3; dr<=3; dr++) {
        const x=Math.round(cx+Math.cos(rd)*(r+dr)), y=Math.round(cy+Math.sin(rd)*(r+dr))
        if (x>1&&y>1&&x<W-1&&y<H-1) t[y][x] = T(base)
      }
    }
  }
  // Santuário central
  disc(t, cx, cy, 6, f===1?'sky_platform':'crystal_floor')
  t[cy][cx] = T(f===1?'fountain':'soul_fire')
  // Cristais ao redor do santuário
  for (let a=0; a<360; a+=45) {
    const rd=a*Math.PI/180
    const x=Math.round(cx+Math.cos(rd)*8), y=Math.round(cy+Math.sin(rd)*8)
    if (x>1&&y>1&&x<W-1&&y<H-1) t[y][x] = T(f===1?'crystal':'dark_crystal')
  }
  // Bruma decorativa (cobweb-like)
  for (let i=0; i<200; i++) {
    const x=2+Math.floor(rand()*(W-4)), y=2+Math.floor(rand()*(H-4))
    if (t[y][x].type === base) t[y][x] = T(f===1?'sky_void':'void')
  }
  border(t, W, H, f===1?'sky_platform':'abyss_wall')
  portalAt(t, 8, 8, 'portal', base)
  if (f===1) portalAt(t, W-9, H-9, 'crystal_portal', 'sky_platform')
  const pool: MonsterType[] = f===1
    ? ['ghost','witch','demon','vampire']
    : ['demon','dragon','vampire','echo_specter' as MonsterType]
  const monsters = spawnPack(t, rand, 95, f===1?55:68, pool, 0.2)
  if (f===2) monsters.push(createMonster('demon', 80, cx*32, cy*32, 'boss'))
  return { id: `mistshrine${f}`, name: `Santuário das Brumas — Andar ${f}`, width: W, height: H, tiles: t, monsters,
    spawnPoints: [{x:10*32, y:10*32}], ambience: f===1?'sky':'abyss', musicTheme: f===1?'sky':'abyss',
    minLevel: f===1?55:68 }
}
...
const BIOME_FNS: Record<string, (f: 1|2) => GameMap> = {
  ...
  roots: genRootGrove,
  solhaven: genSunHaven,
  mistshrine: genMistShrine,
}

...
import { calculateXpToNext, recalcStats, ITEMS, createMonster, generateMap, BASE_STATS, START_WEAPONS, makeClassProgress } from './data'
...
export function damageMonster(
  ...
  if (newHp <= 0) {
    ...
    if (!wasAlreadyDead) {
      ...
      let gems = (newPlayer.gems ?? 0) + gemsGain
      ...
      const existingProgress = newPlayer.classProgress?.[newPlayer.class] ?? makeClassProgress(newPlayer.class)
...
export function updateMonsterAI(state: GameState): GameState {
  ...
  const monsters = map.monsters.map((m, idx) => {
    ...
    if (m.behavior && m.behavior !== 'basic') {
      ...
      // Turret: imóvel, ataca à distância sem perseguir
      if (m.behavior === 'turret') {
        updated.speed = 0
        if (updated.isAggrod && targetDist <= (m.attackRange ?? 200) && updated.attackCooldown === 0) {
          updated.attackCooldown = 60
          const ang = Math.atan2(tdy, tdx)
          const elem = m.element ?? 'physical'
          const pType = elem === 'fire' ? 'fireball' : elem === 'ice' ? 'frost' : elem === 'lightning' ? 'magic' : 'arrow'
          pendingProjectiles.push({
            id: `proj_${Date.now()}_${Math.random()}`,
            x: m.position.x + 16, y: m.position.y + 16,
            vx: Math.cos(ang) * 6, vy: Math.sin(ang) * 6,
            speed: 6, damage: m.attack,
            isCrit: false, radius: 4,
            color: elem === 'lightning' ? '#ffff80' : elem === 'fire' ? '#ff4400' : elem === 'ice' ? '#80c0ff' : '#c0c0c0',
            type: pType as any, pierce: false, hitIds: [], owner: 'monster', life: 120,
          } as any)
        }
      }
      // Charger: investida em linha reta quando cooldown zera
      if (m.behavior === 'charger') {
        updated.chargeCooldown = Math.max(0, (m.chargeCooldown ?? 0) - 1)
        if (updated.isAggrod && updated.chargeCooldown === 0 && targetDist > 48 && targetDist < 400) {
          updated.chargeCooldown = 300
          const ang = Math.atan2(tdy, tdx)
          const dist = Math.min(targetDist, 200)
          const nx = m.position.x + Math.cos(ang) * dist
          const ny = m.position.y + Math.sin(ang) * dist
          const tx = Math.floor((nx + 16) / 32), ty = Math.floor((ny + 16) / 32)
          if (tx >= 0 && ty >= 0 && tx < map.width && ty < map.height && map.tiles[ty][tx].walkable) {
            updated.position = { x: nx, y: ny }
            updated._chargeDamage = true
          }
        }
      }
      // Summoner: invoca aliados periodicamente
      if (m.behavior === 'summoner') {
        updated.summonCooldown = Math.max(0, (m.summonCooldown ?? 0) - 1)
        if (updated.isAggrod && updated.summonCooldown === 0) {
          const maxSummons = 3
          const currentSummons = monsters.filter(om => om._summonedBy === m.id && !om.isDead).length
          if (currentSummons < maxSummons) {
            updated.summonCooldown = 360
            const summonType = m.type === 'broodmother' ? 'spider' : 'skeleton'
            const angle = Math.random() * Math.PI * 2
            const sx = m.position.x + Math.cos(angle) * 48
            const sy = m.position.y + Math.sin(angle) * 48
            try {
              const summoned = createMonster(summonType, Math.max(1, m.level - 3), sx, sy, 'normal')
              summoned._summonedBy = m.id
              pendingSpawns.push(summoned)
            } catch {}
          }
        }
      }
      // Kiter: mantém distância, ataca à distância com debuff
      if (m.behavior === 'kiter') {
        if (updated.isAggrod && targetDist < m.attackRange * 0.5) {
          // Foge do jogador
          const ang = Math.atan2(-tdy, -tdx)
          updated._kiteDx = Math.cos(ang)
          updated._kiteDy = Math.sin(ang)
        } else if (updated.isAggrod && targetDist > m.attackRange * 0.8) {
          // Aproxima
          const ang = Math.atan2(tdy, tdx)
          updated._kiteDx = Math.cos(ang)
          updated._kiteDy = Math.sin(ang)
        ...
        if (updated.isAggrod && targetDist <= (m.attackRange ?? 200) && updated.attackCooldown === 0) {
          updated.attackCooldown = 80
          const ang = Math.atan2(tdy, tdx)
          pendingProjectiles.push({
            id: `proj_${Date.now()}_${Math.random()}`,
            x: m.position.x + 16, y: m.position.y + 16,
            vx: Math.cos(ang) * 5, vy: Math.sin(ang) * 5,
            speed: 5, damage: m.attack,
            isCrit: false, radius: 4,
            color: '#a0ff40', type: 'magic' as any, pierce: false, hitIds: [], owner: 'monster', life: 120,
          } as any)
        }
      }
      // Tank: imóvel, alta defesa, reflete dano
      if (m.behavior === 'tank') {
        updated.speed = 0
        updated._thorns = true
      }
    }
    ...
    if (updated.isAggrod) {
      ...
      if (inAttackRange && !wantsToKite) {
        ...
        // Kiter behavior: usa direção de kite calculada
        if (m.behavior === 'kiter' && (updated._kiteDx || updated._kiteDy)) {
          moveX = updated._kiteDx! * 100
          moveY = updated._kiteDy! * 100
        }
  ...
  if (pendingDmgToPlayer > 0) {
    ...
    // Defense skill gains XP when the player takes damage (skill index 1 = Defesa)
    if (newPlayer.skills && newPlayer.skills[1]) {
      const defSkill = { ...newPlayer.skills[1] }
      defSkill.xp = (defSkill.xp ?? 0) + Math.max(1, Math.round(pendingDmgToPlayer * 0.5))
      // Level up defense skill
      while (defSkill.xp >= defSkill.xpToNext && defSkill.level < 99) {
        defSkill.xp -= defSkill.xpToNext
        defSkill.level += 1
        defSkill.xpToNext = Math.round(defSkill.xpToNext * 1.15)
      }
      const newSkills = [...newPlayer.skills]
      newSkills[1] = defSkill
      newPlayer = { ...newPlayer, skills: newSkills }
    }
  }
  ...
  // HARD CAP: limitar o array de monstros para evitar crash de performance
  // Manter no máximo 600 monstros — prioriza os vivos e mais próximos do jogador
  const MAX_MONSTERS = 600
  if (mergedMonsters.length > MAX_MONSTERS) {
    const alive = mergedMonsters.filter(m => !m.isDead)
    const dead = mergedMonsters.filter(m => m.isDead)
    // Keep all alive up to MAX_MONSTERS, trim dead ones
    if (alive.length >= MAX_MONSTERS) {
      mergedMonsters = alive.slice(0, MAX_MONSTERS)
    } else {
      mergedMonsters = [...alive, ...dead.slice(0, MAX_MONSTERS - alive.length)]
    }
  }
  ...
  mergedMonsters = mergedMonsters.filter((m, idx) => {
    if (m.isDead && m.deathTimer <= 0) {
      ...
      // Se respawn demorou mais de 1200 ticks (20s), remove (era 2400)
      if (respawnElapsed > 1200) {
        return false
      }
      
      // Se player está muito longe do spawn, remove para economizar memória (era 1200px)
    ...
    if (!portalTarget && (isGenericPortal || isBiomePortal)) {
      ...
      if (isGenericPortal) {
        ...
        // Convertemos as coords de tile do mundo para coords "raw" (pré-expansão) antes do range-check.
          ...
          // NOVOS biomas à direita da floresta (Lv50+, 2 andares cada)
          else if (tx >= 578 && tx <= 582 && ty >= 148 && ty <= 152) portalTarget = 'solhaven1'
          else if (tx >= 678 && tx <= 682 && ty >= 348 && ty <= 352) portalTarget = 'mistshrine1'
        ...
        // NOVOS biomas leste (Lv50+) — descida para andar 2
        else if (curId === 'solhaven1')   portalTarget = 'solhaven2'
        else if (curId === 'mistshrine1') portalTarget = 'mistshrine2'
...
export function switchClass(state: GameState, newClass: CharacterClass): GameState {
  ...
  const currentProgress = player.classProgress?.[player.class] ?? makeClassProgress(player.class)
...
export function loadGame(): { player: Player; mapId: string } | null {
  try {
    ...
    // ── Migrate: ensure all CharacterClass keys exist in classProgress ──────
    const ALL_CLASSES: CharacterClass[] = [
      'knight','archer','mage','necromancer','paladin','berserker','assassin','druid','monk',
      'samurai','summoner','alchemist','chronomancer','beastmaster','ninja','pyromancer',
      'cryomancer','stormcaller','geomancer','bard','gunner','templar','warlock','valkyrie',
    ]
    const cp = save.player.classProgress ?? {}
    for (const cls of ALL_CLASSES) {
      if (!cp[cls]) cp[cls] = makeClassProgress(cls)
    }
    save.player.classProgress = cp

    // ── Migrate: ensure pets.partySlots exists ───────────────────────────────
    if (save.player.pets && !Array.isArray(save.player.pets.partySlots)) {
      save.player.pets.partySlots = [save.player.pets.active ?? null, null, null]
    }

    // ── Migrate: ensure inventory is exactly 30 slots ──────────────────────
    if (!Array.isArray(save.player.inventory)) {
      save.player.inventory = Array(30).fill(null)
    } else if (save.player.inventory.length < 30) {
      save.player.inventory = [
        ...save.player.inventory,
        ...Array(30 - save.player.inventory.length).fill(null),
      ]
    }
...
export function updatePets(state: GameState): GameState {
  ...
  if (!pets) return state
  // Guard: ensure partySlots exists (migration for old saves)
  if (!Array.isArray(pets.partySlots)) {
    return {
      ...state,
      player: {
        ...state.player,
        pets: { ...pets, partySlots: [pets.active ?? null, null, null] },
      },
    }
  }

...
export const ABILITIES: Record<string, AbilityDef> = {
  ...
  // ─── NOVAS HABILIDADES (1 por classe, slot 5) ───────────────────────────
  shield_bash:        { id:'shield_bash',        name:'Esmagamento de Escudo', cls:'knight',       description:'Avança em curto alcance impactando inimigos com força brutal.',           icon:'SH', color:'#ffb060', manaCost:20, cooldown:300, unlockLevel:12, effect:'dash',             damageMultiplier:2.6, range:120, radius:55 },
  piercing_arrow:     { id:'piercing_arrow',     name:'Flecha Perfurante',     cls:'archer',       description:'Flecha sagrada que explode ao impacto perfurando até o final.',          icon:'PA', color:'#a0ff80', manaCost:24, cooldown:360, unlockLevel:12, effect:'projectile',       damageMultiplier:3.4, range:420, aoeRadius:60 },
  arcane_orb:         { id:'arcane_orb',         name:'Orbe Arcano',           cls:'mage',         description:'Conjura um orbe instável que detona em pulso arcano massivo.',           icon:'AO', color:'#c080ff', manaCost:30, cooldown:420, unlockLevel:12, effect:'target_aoe',       damageMultiplier:3.2, range:300, radius:130 },
  bone_storm:         { id:'bone_storm',         name:'Tempestade Óssea',      cls:'necromancer',  description:'Esquírolas ósseas giram ao redor cortando tudo perto.',                  icon:'BS', color:'#e0d090', manaCost:28, cooldown:420, unlockLevel:12, effect:'nova',             damageMultiplier:2.4, range:0,   radius:150 },
  sanctuary:          { id:'sanctuary',          name:'Santuário',             cls:'paladin',      description:'Cura intensa e remove pressão sobre o paladino.',                        icon:'SC', color:'#fff080', manaCost:32, cooldown:480, unlockLevel:12, effect:'heal',             damageMultiplier:0,   range:0,   healPercent:0.55 },
  frenzy:             { id:'frenzy',             name:'Frenesi de Sangue',     cls:'berserker',    description:'Entra em frenesi: dano e velocidade aumentam massivamente.',             icon:'FR', color:'#ff4040', manaCost:26, cooldown:540, unlockLevel:12, effect:'buff',             damageMultiplier:0,   range:0,   duration:420 },
  shadowstep:         { id:'shadowstep',         name:'Passo das Sombras',     cls:'assassin',     description:'Salta pelas sombras, retalhando inimigos no caminho.',                   icon:'SS', color:'#8060ff', manaCost:22, cooldown:300, unlockLevel:12, effect:'dash',             damageMultiplier:3.0, range:220, radius:45 },
  natures_blessing:   { id:'natures_blessing',   name:'Bênção da Natureza',    cls:'druid',        description:'Vinhas curam profundamente o druida.',                                    icon:'NB', color:'#80ff80', manaCost:30, cooldown:420, unlockLevel:12, effect:'heal',             damageMultiplier:0,   range:0,   healPercent:0.5 },
  spinning_kick:      { id:'spinning_kick',      name:'Chute Rodopiante',      cls:'monk',         description:'Gira chutes em todas as direções acertando vários inimigos.',            icon:'SK', color:'#ffd070', manaCost:18, cooldown:240, unlockLevel:12, effect:'melee_aoe',        damageMultiplier:2.0, range:0,   radius:100 },
  wind_slash:         { id:'wind_slash',         name:'Corte do Vento',        cls:'samurai',      description:'Três lâminas de vento cortam à distância em leque.',                     icon:'WS', color:'#a0ffff', manaCost:22, cooldown:300, unlockLevel:12, effect:'multi_projectile', damageMultiplier:1.6, range:340, projectileCount:3 },
  spectral_legion:    { id:'spectral_legion',    name:'Legião Espectral',      cls:'summoner',     description:'Invoca uma legião de espectros para combater.',                          icon:'SL', color:'#80a0ff', manaCost:40, cooldown:660, unlockLevel:12, effect:'summon',           damageMultiplier:0,   range:0,   summonCount:4, summonType:'wraith_minion', duration:900 },
  corrosive_cloud:    { id:'corrosive_cloud',    name:'Nuvem Corrosiva',       cls:'alchemist',    description:'Lança uma nuvem ácida que corrói tudo na área.',                         icon:'CC', color:'#80c060', manaCost:26, cooldown:360, unlockLevel:12, effect:'target_aoe',       damageMultiplier:2.6, range:280, radius:110 },
  time_stop:          { id:'time_stop',          name:'Pausa do Tempo',        cls:'chronomancer', description:'Acelera o jogador absurdamente e fortifica defesa.',                     icon:'TS', color:'#ffffff', manaCost:32, cooldown:540, unlockLevel:12, effect:'buff',             damageMultiplier:0,   range:0,   duration:300 },
  eagle_eye:          { id:'eagle_eye',          name:'Olho de Águia',         cls:'beastmaster',  description:'Visão aguçada aumenta alcance e crítico drasticamente.',                 icon:'EE', color:'#ffd060', manaCost:24, cooldown:480, unlockLevel:12, effect:'buff',             damageMultiplier:0,   range:0,   duration:420 },
  kunai_volley:       { id:'kunai_volley',       name:'Salva de Kunais',       cls:'ninja',        description:'Dispara um leque de 7 kunais venenosas.',                                icon:'KV', color:'#404060', manaCost:24, cooldown:300, unlockLevel:12, effect:'multi_projectile', damageMultiplier:1.3, range:300, projectileCount:7 },
  meteor_strike:      { id:'meteor_strike',      name:'Impacto Meteórico',     cls:'pyromancer',   description:'Despenca um meteoro queimando tudo na zona alvo.',                       icon:'MT', color:'#ff7020', manaCost:38, cooldown:540, unlockLevel:12, effect:'target_aoe',       damageMultiplier:3.8, range:320, radius:140 },
  frozen_orb:         { id:'frozen_orb',         name:'Orbe Congelante',       cls:'cryomancer',   description:'Esfera de gelo que explode em estilhaços ao redor.',                     icon:'FO', color:'#a0e0ff', manaCost:28, cooldown:360, unlockLevel:12, effect:'projectile',       damageMultiplier:2.8, range:360, aoeRadius:80 },
  thunder_dome:       { id:'thunder_dome',       name:'Domo do Trovão',        cls:'stormcaller',  description:'Cria um domo de raios em volta do jogador.',                             icon:'TD', color:'#c080ff', manaCost:34, cooldown:480, unlockLevel:12, effect:'nova',             damageMultiplier:3.0, range:0,   radius:160 },
  avalanche:          { id:'avalanche',          name:'Avalanche',             cls:'geomancer',    description:'Pedregulhos massivos despencam na área alvo.',                           icon:'AV', color:'#a07050', manaCost:30, cooldown:420, unlockLevel:12, effect:'target_aoe',       damageMultiplier:3.2, range:280, radius:130 },
  symphony:           { id:'symphony',           name:'Sinfonia Heroica',      cls:'bard',         description:'Música que aumenta ataque e crítico de forma extraordinária.',           icon:'SY', color:'#ff80e0', manaCost:28, cooldown:480, unlockLevel:12, effect:'buff',             damageMultiplier:0,   range:0,   duration:420 },
  gatling_burst:      { id:'gatling_burst',      name:'Rajada Gatling',        cls:'gunner',       description:'Dispara 9 balas em rajada incessante.',                                  icon:'GB', color:'#c0c0c0', manaCost:30, cooldown:360, unlockLevel:12, effect:'multi_projectile', damageMultiplier:1.1, range:340, projectileCount:9 },
  holy_nova:          { id:'holy_nova',          name:'Nova Sagrada',          cls:'templar',      description:'Onda de luz purificadora destrói inimigos ao redor.',                    icon:'HN', color:'#fffac0', manaCost:32, cooldown:480, unlockLevel:12, effect:'nova',             damageMultiplier:3.4, range:0,   radius:150 },
  soul_drain:         { id:'soul_drain',         name:'Sucção de Almas',       cls:'warlock',      description:'Drena vidas em uma grande área, recuperando HP enorme.',                 icon:'SD', color:'#9020c0', manaCost:30, cooldown:420, unlockLevel:12, effect:'life_drain',       damageMultiplier:2.4, range:0,   radius:150, healPercent:0.6 },
  ascension:          { id:'ascension',          name:'Ascensão',              cls:'valkyrie',     description:'Ascende espiritualmente, aumentando ataque e defesa muito.',             icon:'AS', color:'#fff8c0', manaCost:30, cooldown:540, unlockLevel:12, effect:'buff',             damageMultiplier:0,   range:0,   duration:420 },
}


// Lista ordenada de habilidades por classe
export const CLASS_ABILITIES: Record<CharacterClass, string[]> = {
  knight: ['whirlwind', 'charge', 'war_cry', 'earthquake', 'shield_bash'],
  archer: ['power_shot', 'multi_shot', 'evasion', 'arrow_rain', 'piercing_arrow'],
  mage: ['fireball', 'frost_nova', 'arcane_heal', 'meteor', 'arcane_orb'],
  necromancer: ['raise_dead', 'bone_spear', 'life_drain', 'death_nova', 'bone_storm'],
  paladin: ['holy_strike', 'divine_shield', 'lay_on_hands', 'judgment', 'sanctuary'],
  berserker: ['cleave', 'blood_rage', 'reckless_charge', 'bloodbath', 'frenzy'],
  assassin: ['shadow_strike', 'smoke_bomb', 'poison_dagger', 'death_mark', 'shadowstep'],
  druid: ['thorn_lash', 'rejuvenate', 'entangle', 'wild_storm', 'natures_blessing'],
  monk: ['flurry_strike', 'iron_body', 'meditate', 'dragon_palm', 'spinning_kick'],
  samurai: ['iaido', 'bushido', 'cherry_blossom', 'thousand_cuts', 'wind_slash'],
  summoner: ['arcane_bolt', 'summon_familiar', 'spirit_swarm', 'soul_link', 'spectral_legion'],
  alchemist: ['acid_flask', 'fire_bomb', 'healing_brew', 'alchemy_storm', 'corrosive_cloud'],
  chronomancer: ['time_lance', 'temporal_rift', 'haste_field', 'paradox_collapse', 'time_stop'],
  beastmaster: ['hunters_mark', 'feral_companion', 'beast_call', 'primal_roar', 'eagle_eye'],
  ninja: ['shuriken_burst', 'shadow_clone', 'vanish', 'thousand_blades', 'kunai_volley'],
  pyromancer: ['flame_lance', 'fire_wall', 'inner_fire', 'phoenix_rise', 'meteor_strike'],
  cryomancer: ['ice_shard', 'blizzard', 'frost_armor', 'absolute_zero', 'frozen_orb'],
  stormcaller: ['lightning_bolt', 'chain_lightning', 'storm_aegis', 'ragnarok', 'thunder_dome'],
  geomancer: ['stone_throw', 'earth_pillar', 'stone_skin', 'tectonic_shock', 'avalanche'],
  bard: ['sonic_note', 'inspiring_song', 'healing_ballad', 'finale', 'symphony'],
  gunner: ['quick_shot', 'shrapnel', 'grenade', 'bullet_storm', 'gatling_burst'],
  templar: ['hammer_strike', 'holy_shield', 'consecration', 'divine_wrath', 'holy_nova'],
  warlock: ['shadow_bolt', 'curse', 'summon_imp', 'soul_burn', 'soul_drain'],
  valkyrie: ['divine_spear', 'winged_charge', 'warcry_heaven', 'judgement_sky', 'ascension'],
}
...
export function getBuffForAbility(abilityId: string): { name: string; stat: keyof import('./types').CharacterStats; amount: number; duration: number }[] {
  switch (abilityId) {
    ...
    case 'frenzy':
      return [
        { name: 'Frenesi de Sangue', stat: 'attack', amount: 45, duration: 420 },
        { name: 'Frenesi de Sangue', stat: 'speed',  amount: 2,  duration: 420 },
      ]
    case 'time_stop':
      return [
        { name: 'Pausa do Tempo', stat: 'speed',     amount: 4,  duration: 300 },
        { name: 'Pausa do Tempo', stat: 'defense',   amount: 30, duration: 300 },
      ]
    case 'eagle_eye':
      return [
        { name: 'Olho de Águia', stat: 'range',      amount: 80, duration: 420 },
        { name: 'Olho de Águia', stat: 'critChance', amount: 40, duration: 420 },
      ]
    case 'symphony':
      return [
        { name: 'Sinfonia Heroica', stat: 'attack',     amount: 25, duration: 420 },
        { name: 'Sinfonia Heroica', stat: 'critChance', amount: 25, duration: 420 },
      ]
    case 'ascension':
      return [
        { name: 'Ascensão', stat: 'attack',  amount: 30, duration: 420 },
        { name: 'Ascensão', stat: 'defense', amount: 25, duration: 420 },
      ]
    default:
      return []
  }
